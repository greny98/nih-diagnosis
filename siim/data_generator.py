...
def classify_augmentation(training=False):
    if training:
        transform = augment.Compose([
            ...
            augment.RandomSizedCrop(min_max_height=(512, 640), height=IMAGE_SIZE, width=IMAGE_SIZE, w2h_ratio=1.),
        ])
    else:
    ...
    def preprocess_image(image_file):
        ...
        tensor = resnet_v2.preprocess_input(aug_img)
        return tensor
...
def SiimClassificationGenerator(data, image_dir, batch_size=BATCH_SIZE, training=False):
    ...
    ds = ds.shuffle(256 * batch_size, reshuffle_each_iteration=training)
